...
author: "Erola APELLIDO, Olga APELLIDO, Gianluca Arauz, Ramón APELLIDO"
date: "July 10, 2018"
output:
...
1. Paquetes necesarion
1. Delimitación del marco climático ibérico
1. Descarte de variables correlacionadas
1. Reducción de dimensión por Análisis de Componentes Principales (PCA)
1. Oobtención de datos de otros paises y proyección sobre el marco climático ibérico
...
# GIAN -> El " results='hide', message=FALSE, warning=FALSE " sirve para que este chunk no nos escriva el output de cargar la librerías en el HTML. Cosa que no aporta nada y queda feillo en el HTML
...
plot (s.crop$bio1,
   main = "Mapa de la temperatura media (BIO1) en Península Ibérica",
   xlab = "Longitud (ºC)",
   ylab = "Latitud (ºC)")
```
...
plot(marcoclim$BIO1~marcoclim$BIO12, pch=19, col="lightblue", xlab="Precipitacion Anual (mm)", ylab="Temperatura anual media (ÂºC x10)") #ploteamos temperatura media anual y precipitacion anual)
...
#Download the WorldClim Data
worldata <- getData("worldclim",var="bio", res=2.5)
plot(worldata$bio1, main =  "Temperatura media anual mundial (ÂºC x10)")
```
