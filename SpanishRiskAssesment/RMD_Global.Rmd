...
# Valoración del riesgo invasor en la Pen�?nsula Iberica.
...
# 1. PREGUNTAS Y DEFINICIONES INICIALES
# 1.1 Preguntas
Si las invasiones son exitosas por en coincidencia del clima, ¿qué especies estar�?an bien en la pen�?nsula ibérica y son por tanto potencialmente invasoras?

¿Las especies invasoras actuales en la pen�?nsula ibérica cumplen esa hipótesis de coincidencia del clima?
Si las condiciones climáticas en la pen�?nsula cambiasen en el contexto del cambio climático, ¿el numero de especies potencialmente invasoras ser�?a mayor, menor o igual? ¿El cambio climático podr�?a potenciar la amenaza invasora?

Teniendo en cuenta el origen de las especies potencialmente invasoras y los movimientos globales de mercanc�?as y personas, ¿qué invasiones son mas probables? ¿Con qué paises deber�?amos endurecer las pol�?ticas de comercio de especies?

¿Por qué superficie se extender�?an las especies potencialmente invasoras en la pen�?nsula? ¿Cuánto es su daño potencial?

Teniendo en cuenta las condiciones climáticas en la pen�?nsula, ¿qué otros pa�?ses de Europa podr�?an ser susceptibles a la invasión debido a similaridades con el marco climático de España y Portugal?
...
plot (s.crop$bio1,
   main = "Mapa de la variable BIO1 en la Pen�?nsula Ibérica",
   xlab = "Longitud (�C)",
   ylab = "Latitud (�C)")
```
![Gráfico](./Plots/MarcoIbericoMapaBIO1.png)
...
```
![Gráfico](./Plots/correlationplot.png)
...
```{r, fig.cap= "Temperatura y Precipitación en la Pen�?nsula Ibérica"}
...
```
![Gráfico](./Plots/ScatterplotMarcoclim.png)

Para generar un gráfico de densidad:
```{r, fig.cap = "Grafico de densidad de Temperatura y Precipitación en la Pen�?nsula Ibérica"}
str(marcoclim)
...
```
![Gráfico](./Plots/MarcoClimaticoIberico.png)
...
```
![Gráfico](./Plots/ScatterplotPCA.png)
...
Otro plot interesante, es por ejemplo, la contribucion de cada una de las variables en el análisis de componentes principales.
```{r}
...
print(g)
```
![Gráfico](./Plots/ContributionPCA.png)

# 6. OBTENCIÓN DE DATOS DE OTROS PAISES Y PROYECCIÓN SOBRE EL MARCO CLIMÁTICO IBÉRICO
Pretendemos evaluar pa�?s por pa�?s, si sus condiciones climáticas se parecen a las de la pen�?nsula ibérica, con el fin de determinar después especies que podr�?an encontrar condiciones favorables en nuestro pa�?s actuando como invasoras. Por eso proyectamos los datos de marco climático de estos pa�?ses sobre el nuestro.
...
```
![Gráfico](./Plots/MapaDinamarca.png)
...
```
![Gráfico](./Plots/GgplotContrastePaises.png)
...
```
![Gráfico](./Plots/PCAMarcoClimIberico.png)
![Gráfico](./Plots/PCAdensityDinamarca.png)
