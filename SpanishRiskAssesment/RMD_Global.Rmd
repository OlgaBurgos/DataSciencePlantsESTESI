...
author: "Erola APELLIDO, Olga Burgos, Gianluca Arauz, Ram?n APELLIDO"
date: "July 10, 2018"
output:
...
1. Delimitaci?n del marco clim?tico ib?rico
1. Descarte de variables correlacionadas
1. Reducci?n de dimensi?n por An?lisis de Componentes Principales (PCA)
1. Oobtenci?n de datos de otros paises y proyecci?n sobre el marco clim?tico ib?rico
...
Si las condiciones clim?ticas en la pen?nsula cambiasen en el contexto del cambio clim?tico, ?el numero de especies potencialmente invasoras ser?a mayor, menor o igual? ?El cambio clim?tico podr?a potenciar la amenaza invasora?
...
# GIAN -> El " results='hide', message=FALSE, warning=FALSE " sirve para que este chunk no nos escriva el output de cargar la librer?as en el HTML. Cosa que no aporta nada y queda feillo en el HTML
...
plot (s.crop$bio1,
   main = "Mapa de la temperatura media (BIO1) en Pen?nsula Ib?rica",
   xlab = "Longitud (?C)",
   ylab = "Latitud (?C)")
```
