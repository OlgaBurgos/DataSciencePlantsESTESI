...
# ValoraciÛn del riesgo invasor en la PenÌnsula IbÈrica.

### ÕNDICE DE CONTENIDOS
### 1. PREGUNTAS Y DEFINICIONES INICIALES
### 2. PAQUETES NECESARIOS
### 3. DELIMITACI”N DEL MARCO CLIM¡TICO IB…RICO
### 4. DESCARTAR VARIABLES MUY CORRELACIONADAS
### 5. REDUCCI”N DE DIMENSIONES POR PCA
### 6. OBTENCIÛN DE DATOS DE OTROS PAISES Y PROYECCI”N SOBRE EL MARCO CLIM¡TICO IB…RICO 
### 7. DETERMINACI”N DEL RIESGO INVASOR
### 8. OBTENCION DATOS DE ESPECIES INVASORAS MUNDIALES. ORIGEN Y PRESENCIA
### 9. BIBLIOGRAFÕA

# 1. PREGUNTAS Y DEFINICIONES INICIALES
# 1.1 Preguntas
Si las invasiones son exitosas por en coincidencia del clima, ¬øquÈ especies estar√?an bien en la pen√?nsula ibÈrica y son por tanto potencialmente invasoras?

¬øLas especies invasoras actuales en la pen√?nsula ibÈrica cumplen esa hip√≥tesis de coincidencia del clima?
...
Teniendo en cuenta el origen de las especies potencialmente invasoras y los movimientos globales de mercanc√?as y personas, ¬øquÈ invasiones son mas probables? ¬øCon quÈ paises deber√?amos endurecer las pol√?ticas de comercio de especies?

¬øPor quÈ superficie se extender√?an las especies potencialmente invasoras en la pen√?nsula? ¬øCu√°nto es su da√±o potencial?

Teniendo en cuenta las condiciones clim√°ticas en la pen√?nsula, ¬øquÈ otros pa√?ses de Europa podr√?an ser susceptibles a la invasi√≥n debido a similaridades con el marco clim√°tico de Espa√±a y Portugal?

# 1.2. Definiciones
Marco climatico IbÈrico: espacio multidimensional definido por los valores de diferentes variables bioclim√°ticas georeferenciados dentro de la Peninsula IbÈrica.
...
Especie potencialmente invasora en la Peninsula IbÈrica: especie cuyo nicho clim√°tico est√° comprendido dentro del Marco clim√°tico IbÈrico.
...
# 3. DELIMITACIÛN DEL MARCO CLIM·TICO IBÈRICO
...
Los datos se descargan en formato raster. Ahora hay que seleccionar los datos correspondientes a la Pen√?nsula IbÈrica. Realizamos la selecci√≥n recortando el cuadrado en el cual se comprende la peninsula. 
```{r}
...
plot (s.crop$bio1,
   main = "Mapa de la variable BIO1 en la Pen√?nsula IbÈrica",
   xlab = "Longitud (∫C)",
   ylab = "Latitud (∫C)")
```
...
```{r}
write.csv(marcoclim, file="Datos/MarcoClim2.5.csv") #Guardamos los datos
```


!!!!!!!!! PARA REEMPRENDER DESDE AQUÕ, CARGA LOS DATOS !!!!!!!!!!!
```{r}
...
```{r, fig.cap= "Temperatura y Precipitaci√≥n en la Pen√?nsula IbÈrica"}
...
```{r, fig.cap = "Grafico de densidad de Temperatura y Precipitaci√≥n en la Pen√?nsula IbÈrica"}
str(marcoclim)
...
filled.contour(dens,
               ...
               ylab= "Annual Precipitation (mm)", main = "Marco Clim√°tico IbÈrico")
...
# 5. REDUCCIÛN DE DIMENSIONES POR PCA
PAra describir de forma reducida el marco clim√°tico IbÈrico realizamos una reducci√≥n de dimensiones por Analisis de Componentes principales:
...
# 6. OBTENCIÛN DE DATOS DE OTROS PAISES Y PROYECCIÛN SOBRE EL MARCO CLIM·TICO IBÈRICO
Pretendemos evaluar pa√?s por pa√?s, si sus condiciones clim√°ticas se parecen a las de la pen√?nsula ibÈrica, con el fin de determinar despuÈs especies que podr√?an encontrar condiciones favorables en nuestro pa√?s actuando como invasoras. Por eso proyectamos los datos de marco clim√°tico de estos pa√?ses sobre el nuestro.

Cargamos los datos del marco climatico ibÈrico:
```{r}
marcoclim<- read_csv("Datos/MarcoClim2.5.csv", 
    col_types = cols(X1 = col_skip()))
...
Preparamos los datos de la Peninsula IbÈrica de los dos primeros componentes:
```{r}
e <- pcaclim$x[,1:2]
edf <- as.data.frame(e)
...
Proyectamos estos valores sobre el PCA del marco climatico ibÈrico
```{r}
...
```{r}
# PARA HACER UN SOLO GRAFICO DE DENSIDAD CON TODOS LOS DATOS SERIA ASÕ
...
filled.contour(dense,
               color.palette=colorRampPalette(c('gray94','blue','yellow','red','darkred')),
               xlab= "PC1",
               ylab= "PC2", main = "Marco clim√°tico IbÈrico")
...
filled.contour(densi,
               color.palette=colorRampPalette(c('gray94','blue','yellow','red','darkred')),
               xlab= "PC1",
               ylab= "PC2", main = "Marco clim√°tico IbÈrico del pais proyectado")
...
# 7. DETERMINACIÛN DEL RIESGO INVASOR
